...
export default function AdminDepositsPage() {
  ...
  const fetchDeposits = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        console.error("Deposits data is not an array:", data);
        setDeposits([]);
      }
    } catch (err) {
  ...
  const handleAction = async (depositId: string, status: string) => {
    console.log(`🔄 Button clicked: ${status} for deposit ${depositId}`);
    ...
    try {
      console.log(`📤 Sending ${status} request for deposit:`, depositId);
  ...
  const pendingCount = Array.isArray(deposits)
                  ...
                  <td className="px-4 py-3">
                    {deposit.status === "pending" && (
                      <div className="flex gap-2">
                        <button
                          onClick={() => {
                            console.log("🔵 Approve clicked for:", deposit.id);
                            handleAction(deposit.id, "approved");
                          }}
                          disabled={actionLoading === deposit.id}
                          className="rounded-lg bg-green-600 px-3 py-1 text-xs font-semibold text-white hover:bg-green-700 disabled:opacity-50 cursor-pointer"
                        >
                          {actionLoading === deposit.id ? "Loading..." : "Approve"}
                        </button>
                        <button
                          onClick={() => {
                            console.log("🔴 Reject clicked for:", deposit.id);
                            handleAction(deposit.id, "rejected");
                          }}
                          disabled={actionLoading === deposit.id}
                          className="rounded-lg bg-red-600 px-3 py-1 text-xs font-semibold text-white hover:bg-red-700 disabled:opacity-50 cursor-pointer"
                        >
                          {actionLoading === deposit.id ? "Loading..." : "Reject"}
                        </button>
                      </div>
                    )}
                  </td>
      ...
      {showScreenshot && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setShowScreenshot(null)}>
          <div className="max-w-2xl w-full bg-[#0f213d] rounded-xl border border-[#1e3a66] p-4" onClick={(e) => e.stopPropagation()}>
            ...
            {deposits.find(d => d.id === showScreenshot) && (
              <img
