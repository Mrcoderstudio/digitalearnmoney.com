...
export const withdrawals = pgTable("withdrawals", {
  ...
  userId: uuid("user_id").references(() => users.id).notNull(),
  amount: numeric("amount", { precision: 20, scale: 2 }).notNull(),
  method: varchar("method", { length: 50 }).notNull(), // jazzcash, easypaisa, bank
  accountDetails: jsonb("account_details").notNull(), // { bankName, accountNumber, accountHolder }
  status: varchar("status", { length: 20 }).default("pending").notNull(),
  adminNote: text("admin_note"),
...
// ============================================
// SETTINGS TABLE (Easypaisa Account Details)
// ============================================
export const settings = pgTable("settings", {
  ...
  whatsappChannelLink: text("whatsapp_channel_link").default("https://whatsapp.com/channel/0029VbDvcyeHLHQbpskXgI28"),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
